...
Fig5 <- function(){
	...
	par("xaxs"="i","yaxs"="i")
	plot(NULL,type="n",xlim=c(0,100),ylim=c(0,100),ylab="\\% in school total pop",xlab="\\% with child enrolled")
	extr <- par("usr")
	...
	for (i in 1:length(ages)){
		...
		y <- 100*(1-DATA[DATA$age==ages[i] & DATA$sex=="Female" & DATA$year>=1998,"prop_school"])
