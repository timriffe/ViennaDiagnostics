% setwd("C:\\Users\\triffe\\git\\ViennaPaper\\ALBERTPAPERS") ; getwd()
...
DATA <- read.table("DATA\\FIGURE1_2.txt",header=T,sep="\t",na.strings = ".")
...
% the plot and its products are defined within a function in order to speed along the cache on rebuilds.
% bit by bit I'll redefine the other figures in this way: doesn't take that long
<<>>=
f1 <- function(){
	omar <- par("mar")
...
\begin{figure}
<<fig=TRUE>>=
plot(12:24,QuantilesMat1[,4],type="s",xlab="age",ylab="observations",col="#7D26CD",ylim=c(0,130))
...
% this plot is particularly large, and takes some time to insert...
<<>>=
f3 <- function(){
	par("xaxs"="i","yaxs"="i",mar=c(8,4,3,2))
	plot(NULL,type="n",xlim=c(11.5,24.5),ylim=c(0,100),ylab="\\% population",xlab="Age")
...
cty <- f7()
@
\caption{Females, 2000, \% Enrolled vs \% in union, 3 ages (\Sexpr{cty} country observations)}
...
\begin{figure}
<<fig=TRUE>>=
plot(12:24,f8out$sps,type='l',ylim=c(-3.5,0),xlab="age",ylab="First Derivative")
polygon(c(12:24,24:12),c(f8out$sps-1.96*f8out$sdev,rev(f8out$sps+1.96*f8out$sdev)),col="#00000030")
...
cty9 <- f9
@
\caption{Females, 2000, \% Enrolled vs \% mother, 3 ages (\Sexpr{cty9} country observations)}
...
\begin{figure}
<<fig=TRUE>>=
plot(12:24,f10out$sps,type='l',ylim=c(-3.5,1),xlab="age",ylab="First Derivative")
polygon(c(12:24,24:12),c(f10out$sps-1.96*f10out$sdev,rev(f10out$sps+1.96*f10out$sdev)),col="#00000030")
abline(h=c(0,-.5,-1,-2),lty=2)
@
\caption{Females, 2000, \% Enrolled vs \% in union, change in slope of age-specific OLS}
...
cty11 <- f11()
@
\caption{Males, 2000, \% Enrolled vs \% in union, 3 ages (\Sexpr{cty11} country observations)}
...
\begin{figure}
<<fig=TRUE>>=
plot(12:24,sps,type='l',ylim=c(-3.5,1),xlab="age",ylab="First Derivative")
...
DATA <- read.table("DATA/FIGURE3.txt",header=T,sep="\t",na.strings = ".")
...
<<>>=
DATA2 <- read.table("DATA\\FIGURE1_2.txt",header=T,sep="\t",na.strings = ".")
...
par(new=T)
plot(12:24,obs,axes=F,type="s",col="red",lwd=2,ylim=c(0,130),ylab="",xlab="")
axis(4)
legend(12,-12,col=c("green","red"),lwd=2,legend=c("r2 (left)","observations (right)"),xpd=TRUE)
@
\caption{Females, 2000, \% Enrolled vs \% in union, change in slope of age-specific OLS}
